defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>Live.Index do
  ...
  def mount(_params, session, socket) do
    ...
    {:ok, assign(socket, :<%= schema.collection %>, list_<%= schema.plural %>(socket.assigns.site_id))}
  end
  ...
  defp list_<%= schema.plural %>(site_id) do
    <%= inspect context.alias %>.list_<%= schema.plural %>(site_id)
  end
end
